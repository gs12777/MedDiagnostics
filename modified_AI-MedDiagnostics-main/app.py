...
import DiseasePred
import openai
openai.api_key = "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"
messages = []
...
def HealthCareAssistant():
    message=""
    reply="Hello, I am your personal AI healthcare assistant"
    ...
    if request.method == "POST":
        ...
        usr_inp = request.form
        if usr_inp["message"]!="":
            message = usr_inp["message"]
            print(message)
            ...
            print("\n" + reply + "\n")
    return render_template("HealthCareAssistant.html", reply=reply, message=message)
...
def Disease():
    ...
    if request.method == "POST":
        ...
        if len(symptoms) < 5 or len(symptoms) > 8:
            ...
            prediction, medicines = DiseasePred.predicts(symptoms)
            if prediction:
